		...
		while let Poll::Ready(event) = Pin::new(&mut self.inner).poll_next(cx) {
			...
			match event {
				Some(Ok(Event::Open)) => {
					...
					if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/anthropic_sse_debug.log") {
						use std::io::Write;
						let _ = writeln!(f, "[OPEN] EVENT::OPEN (stream started)");
					}
					return Poll::Ready(Some(Ok(InterStreamEvent::Start)));
				}
				Some(Ok(Event::Message(message))) => {
					let message_type = message.event.as_str();
					// Debug: write SSE events to file for diagnosis
					if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/anthropic_sse_debug.log") {
						use std::io::Write;
						let data_preview = &message.data[..message.data.len().min(1000)];
						let _ = writeln!(f, "[MSG] type='{}' data='{}'", message_type, data_preview);
					}
					...
					match message_type {
						...
						"error" => {
							if let Ok(error_body) = take_stream_error(&message.data, &self.options.model_iden) {
								return Poll::Ready(Some(Err(Error::ChatResponse {
									model_iden: self.options.model_iden.clone(),
									body: error_body,
								})));
							}
							// If parsing fails, surface as StreamParse error with the raw data
							return Poll::Ready(Some(Err(Error::StreamParse {
								model_iden: self.options.model_iden.clone(),
								serde_error: serde_json::Error::io(std::io::Error::new(
									std::io::ErrorKind::Other,
									format!("Anthropic error event: {}", message.data),
								)),
							})));
						}
				...
				Some(Err(err)) => {
					...
					if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/anthropic_sse_debug.log") {
						use std::io::Write;
						let _ = writeln!(f, "[ANTHROPIC-STREAMER] Error from EventSourceStream: {}", err);
					}
					log::error!("[anthropic-streamer] SSE error event: {}", err);
				...
				None => {
					...
					if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/anthropic_sse_debug.log") {
						use std::io::Write;
						let _ = writeln!(f, "[NONE] EVENT::NONE (stream ended)");
					}
					return Poll::Ready(None);
				}
			}
...
/// Extract the error body from an Anthropic SSE error event.
/// Format: `{"type": "error", "error": {"type": "...", "message": "..."}}`
fn take_stream_error(data: &str, model_iden: &ModelIden) -> std::result::Result<Value, Error> {
	let mut value: Value = serde_json::from_str(data).map_err(|serde_error| Error::StreamParse {
		model_iden: model_iden.clone(),
		serde_error,
	})?;
	value.x_take::<Value>("error").map_err(|_| Error::StreamParse {
		model_iden: model_iden.clone(),
		serde_error: serde_json::Error::io(std::io::Error::new(
			std::io::ErrorKind::Other,
			"Anthropic error event missing 'error' field",
		)),
	})
}
