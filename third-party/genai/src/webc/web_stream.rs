				...
				match Pin::new(fut).poll(cx) {
					Poll::Ready(Ok(response)) => {
						...
						if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/anthropic_sse_debug.log") {
							use std::io::Write;
							let _ = writeln!(f, "[WEB] HTTP status: {} {}", status.as_u16(), status.canonical_reason().unwrap_or("?"));
						}
						if !status.is_success() {
							...
							let error_future = async move {
								...
								if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/anthropic_sse_debug.log") {
									use std::io::Write;
									let _ = writeln!(f, "[WEB] Error body: {}", &body[..body.len().min(2000)]);
								}
					...
					Poll::Ready(Err(e)) => {
						this.response_future = None;
						// Debug: log error propagated to stream
						if let Ok(mut f) = std::fs::OpenOptions::new().create(true).append(true).open("/tmp/anthropic_sse_debug.log") {
							use std::io::Write;
							let _ = writeln!(f, "[WEB] Error propagated to stream: {}", e);
						}
